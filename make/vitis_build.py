...
# if domain_name != "standalone_ps7_cortexa9_0" : raise Exception("NOT EQUAL")
...
# Create the platform
platform = client.create_platform_component(name = config.platform_name,hw_design = sys.argv[2] ,os = config.os_name,cpu = config.cpu_name, no_boot_bsp = False)
# Add the bsp libs
if config.bsp_libs != None:
    platform = client.get_component(config.platform_name)
    domain = platform.get_domain(config.os_name + "_"+ config.cpu_name)
...
comp = client.create_app_component(name=config.app_name,platform =platform_xpfm,domain = domain_name,template = config.template_name)
